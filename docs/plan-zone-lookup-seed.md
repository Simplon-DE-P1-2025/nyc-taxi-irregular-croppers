...
> But : remplacer les `pulocationid` bruts de `zone_analysis` par des noms lisibles (Borough, Zone,
...
|---|---|---|
| `LocationID` | 132 | clé de jointure ↔ `pulocationid` / `dolocationid` |
...
- **c) `zone_analysis` (LEFT JOIN)** : ✅ **aucun impact sur les chiffres**. `locationid` est unique
  ...
  **volontaire** : un trajet avec un `pulocationid` hors lookup reste compté (Borough/Zone = NULL)
...
- **e) Test `relationships`** : ⚠️ **seul vrai risque de build rouge**. Si une donnée a un
  `pulocationid` absent de [1..265], le test échoue (`severity: error` par défaut). Précautions :
...
|---|---|
| `pulocationid = 132` | **JFK Airport**, Borough **Queens**, service_zone **Airports** |
...
dimension se joint aussi sur `dolocationid` (zone de dépose) pour des flux origine→destination.
...
> **Contexte** : exposer Borough/Zone/service_zone au lieu des pulocationid bruts (KPI top zones lisible).
> **DoD** :
